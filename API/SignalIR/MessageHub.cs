...
namespace API.SignalIR
{
    public class MessageHub(IUnitOfWork unitOfWork, IMapper mapper, IHubContext<PresenceHub> presenceHub) : Hub
    {
        ...
        {
            var httppContext = Context.GetHttpContext();
            var otherUser = httppContext?.Request.Query["user"];
            ...
            var messages = await unitOfWork.MessageRepository.GetMessageTheread(Context.User.GetUsername(), otherUser!);
        ...
        public async Task SendMessage(CreateMesssageDto createMesssageDto)
        {
            var username = Context.User?.GetUsername() ?? throw new Exception("cloud not get user");

            if(username == createMesssageDto.RecipientUsername.ToLower()) 
                throw new HubException("You cannot message yourself");

            var sender = await unitOfWork.UserRepository.GetUserByUsernameAsync(username);
            var recipient = await unitOfWork.UserRepository.GetUserByUsernameAsync(createMesssageDto.RecipientUsername);
